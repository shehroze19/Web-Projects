...
if(isset($_POST['checked'])){
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `appointment` SET `finished`='Finished By: $name' WHERE `id`='$update_id'";
...
<tr>
					...
					<th class='bg-primary text-center' >Finish <input type="checkbox" onClick="toggle(this)" />Select All<br/></th>

											<th class='bg-primary text-center'>Prescription</th>
										...
										$result=mysqli_query($con,"SELECT * FROM `appointment` WHERE `doctor_uname`='$Username' and `approved`!=''  ;");
	...
	if($row['finished']==null){ 
						...
						} else { echo "<td>Finished</td>";
							echo "<td><a class='btn btn-danger' style='color:white;' href='prescription.php?id=".$row['id']."'>Prescribe</a></td>";
									...
									<div class="form-group"><input type="Submit" name="checked" value="Finish Selected" class="btn btn-block alert-success"></div>
