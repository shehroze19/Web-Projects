	...
	if(isset($_POST['checked']))
	{
		...
		if(isset($_POST['comments'])){
			...
			for($i=0;$i<count($comments);$i++){
				...
				$update_id = $ids[$i];
				$comment=$comments[$i];
				$sql = "UPDATE `assets_tracking_sheet` SET `comments`='$comment' WHERE `s.no`='$update_id'";
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `assets_tracking_sheet` SET `pending`='Verified By: $name' WHERE `s.no`='$update_id'";
				$result = mysqli_query($con,$sql);
			}
			...
			<form action="" method="post">
				...
				<table id="myTable" class="table-responsive" border="3">
					<tr class="header">
						...
						<th class="text-center active" style="width:5%;">TICKET ID</th>
