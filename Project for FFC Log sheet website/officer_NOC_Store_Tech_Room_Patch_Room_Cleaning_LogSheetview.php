...
<body style="color:white;">
	...
	if(isset($_POST['checked']))
	{
		...
		if(isset($_POST['comments'])){
			...
			for($i=0;$i<count($comments);$i++){
				...
				$update_id = $ids[$i];
				$comment=$comments[$i];
				$sql = "UPDATE `noc_store_tech_room_path_room_cleaning_logsheet` SET `comments`='$comment' WHERE `S_No`='$update_id'";
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `noc_store_tech_room_path_room_cleaning_logsheet` SET `pending`='Verified By: $name' WHERE `S_No`='$update_id'";
				$result = mysqli_query($con,$sql);
	...
	<div class="container-fluid">
		...
		<div class="page-header text-center">
			<h2 style="color:white">NOC Store, Tech Room & PACH ROOM CLEANING LOG SHEET</h2>
		...
		<form action="" method="post">
			<table id="myTable" class="table-responsive" border="3">
				<tr class="header">
					...
					<th class="text-center active"  style="width:10%;">NOC Store</th>
					<th class="text-center active"  style="width:5%;">Tech Room</th>
					<th class="text-center active"  style="width:5%;">Patch Room</th>
					<th class="text-center active"  style="width:10%;">Duty Personal</th>
					<th class="text-center active"  style="width:5%;">Supervised By</th>
					...
					$query2="SELECT * from noc_store_tech_room_path_room_cleaning_logsheet";
					...
					while($row = mysqli_fetch_assoc($result))
					{
						...
						echo "<td class='text-center'>" .  $row['NOC_Store'] . "</td>";
						echo "<td class='text-center'>" .  $row['Tech_Room'] . "</td>";
						echo "<td class='text-center'>" .  $row['Patch_Room'] . "</td>";
						echo "<td class='text-center'>" .  $row['Duty_Personal'] . "</td>";
						echo "<td class='text-center'>" .  $row['Supervised_By'] . "</td>";
