	...
	if(isset($_POST['checked']))
	{
		...
		if(isset($_POST['comments'])){
			...
			for($i=0;$i<count($comments);$i++){
				...
				$sql = "UPDATE `incidental_log_sheet_noc_dc_pr` SET `comments`='$comment' WHERE `s.no`='$update_id'";
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `incidental_log_sheet_noc_dc_pr` SET `pending`='Verified By: $name' WHERE `s.no`='$update_id'";
				$result = mysqli_query($con,$sql);
			}
