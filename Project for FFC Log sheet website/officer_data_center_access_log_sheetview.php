...
<body style="color:white;">
	...
	if(isset($_POST['checked']))
	{
		...
		if(isset($_POST['comments'])){
			...
			for($i=0;$i<count($comments);$i++){
				...
				$update_id = $ids[$i];
				$comment=$comments[$i];
				$sql = "UPDATE `data_center_access_log_sheet` SET `comments`='$comment' WHERE `s.no`='$update_id'";
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `data_center_access_log_sheet` SET `pending`='Verified By: $name' WHERE `s.no`='$update_id'";
				$result = mysqli_query($con,$sql);
			}
	...
	<div class="container-fluid">
		...
		<div class="page-header text-center">
			<h2 style="color:white;">DATA CENTER ACCESS LOG SHEET</h2>
		...
		<form action="" method="post">
			<table id="myTable" class="table-responsive" border="3">
				<tr class="header">
					...
					<th class="text-center active" style="width:5%;">TIME IN</th>
					<th class="text-center active" style="width:15%;">NAME</th>
					<th class="text-center active" style="width:8%;">DEPARTMENT</th>
					<th class="text-center active" style="width:15%;">REASON</th>
					<th class="text-center active" style="width:5%;">TIME OUT</th>
					<th class="text-center active" style="width:20%;">ACCOMPANIED BY</th>
					<th class="text-center active" style="width:20%;">AUTHORIZED BY</th>
					<th class="text-center active" style="width:25%;">REVIEWED BY</th>
					...
					$query2="SELECT * from data_center_access_log_sheet ";
					...
					while($row = mysqli_fetch_assoc($result))
					{
						...
						echo "<td class='text-center'>" . $row['department_section'] . "</td>";
						echo "<td class='text-center'>" . $row['reason'] . "</td>";
						echo "<td class='text-center'>" . str_replace(':00','',$row['time_out']) . "</td>";
						echo "<td class='text-center'>" . $row['accompanied_by'] . "</td>";
						echo "<td class='text-center'>" . $row['authorized_by'] . "</td>";
						echo "<td class='text-center'>" . $row['reviewed_by'] . "</td>";
