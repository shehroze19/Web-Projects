	...
	if(isset($_POST['checked']))
	{
		...
		if(isset($_POST['comments'])){
			...
			for($i=0;$i<count($comments);$i++){
				...
				$update_id = $ids[$i];
				$comment=$comments[$i];
				$sql = "UPDATE `dc_air_conditioning` SET `comments`='$comment' WHERE `s.no`='$update_id'";
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `dc_air_conditioning` SET `pending`='Verified By: $name' WHERE `s.no`='$update_id'";
				$result = mysqli_query($con,$sql);
			}
