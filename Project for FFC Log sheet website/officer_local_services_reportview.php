	...
	if(isset($_POST['checked']))
	{
		...
		if(isset($_POST['comments'])){
			...
			for($i=0;$i<count($comments);$i++){
				...
				$update_id = $ids[$i];
				$comment=$comments[$i];
				$sql = "UPDATE `local_services` SET `additional_comments`='$comment' WHERE `s.no`='$update_id'";
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `local_services` SET `pending`='Verified By: $name' WHERE `s.no`='$update_id'";
				$result = mysqli_query($con,$sql);
			}
			...
			<table id="myTable" class="table-responsive" border="3">
				<tr class="header">
					...
					while($row = mysqli_fetch_assoc($result))
					{
						...
						echo "<td class='text-center'><textarea style='color:black;' name='comments[]' value='" . $row['additional_comments'] . "'>".$row['additional_comments']." </textarea></td>";
