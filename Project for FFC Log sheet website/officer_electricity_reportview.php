...
<body style="color:white;">
	...
	if(isset($_POST['checked']))
	{
		...
		if(isset($_POST['comments'])){
			...
			for($i=0;$i<count($comments);$i++){
				...
				$update_id = $ids[$i];
				$comment=$comments[$i];
				$sql = "UPDATE `electricity_reports` SET `comments`='$comment' WHERE `id`='$update_id'";
		...
		if(isset($_POST['checkbox'])){
			...
			for($i=0;$i<count($checkbox);$i++){
				...
				$sql = "UPDATE `electricity_reports` SET `pending`='Verified By: $name' WHERE `id`='$update_id'";
				$result = mysqli_query($con,$sql);
			}
	...
	<div class="container-fluid">
		...
		<div class="page-header text-center">
			<h2 style="color:white">Electricity Power</h2>
		...
		<form action="" method="post">
			<table id="myTable" class="table-responsive" border="3">
				<tr class="header">
					...
					<th class="text-center active"  style="width:5%;">Electricity State</th>
					<th class="text-center active"  style="width:5%;">Time From</th>
					<th class="text-center active"  style="width:10%;">Time To/th>
						...
						while($row = mysqli_fetch_assoc($result))
						{
							...
							echo "<td>" . str_replace(':00','',$row['time_from']) . "</td>";
							echo "<td>" . str_replace(':00','',$row['time_to']) . "</td>";
							...
							echo "<td class='text-center'>" .  str_replace(':00','',$row['original_time_from']) . "</td>";
