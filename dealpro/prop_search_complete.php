...
<div class="container">
            ...
            <div class="form-group row">
                ...
                if(count($_POST)>0){
                    ...
                    $by_city=$_POST['city'];
                    $by_address=$_POST['address'];
                    $by_type=$_POST['type'];
                    $by_pmin=$_POST['pmin'];
                    $by_pmax=$_POST['pmax'];
                    $by_amin=$_POST['amin'];
                    $by_amax=$_POST['amax'];
                    $by_unit=$_POST['areaunit'];
            $city_result = mysqli_query($con,"SELECT `id` FROM `city` WHERE city_name='$by_city' ");
            $city_row = mysqli_fetch_array($city_result);
            $city_id=$city_row['id'];

            $type_result = mysqli_query($con,"SELECT `id` FROM `plot_types` WHERE type_name='$by_type' ");
            ...
            $type_id=$type_row['id'];
              ...
              $query  = "SELECT * FROM `plots`";
                $conditions = array();
                if(! empty($by_address)) {
                    $conditions[] = "address='$by_address'";
                }
                if(! empty($by_type)) {
                    $conditions[] = "city_id='$by_type'";
                }
                if(! empty($by_email)) {
