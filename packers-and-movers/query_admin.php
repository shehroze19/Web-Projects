...
<tr>
											<th class='bg-primary text-center' >Query Posted By:</th>
											<th class='bg-primary text-center'>Query Description:</th>
											<th class='bg-primary text-center'>Time Posted:</th>
											<th class='bg-primary text-center'>Click here to reply:</th>
											</tr>
										<?php 

										$result=mysqli_query($con,"select * from queries WHERE replied=0 ORDER BY qdate ASC");
										...
										while($row=$data = mysqli_fetch_assoc($result)){
											...
											echo "	
											<tr>
											<td class='text-center'> ".$row['uName']."</td>
											<td class='text-center'> ".$row['query']."</td>
											<td class='text-center'> ".$row['qdate']."</td>
											<td class='text-center'><a href='query_reply_admin.php?query=".$row['qId']."'>Click here to reply</a> </td>
