...
<div class="container-fluid">
    ...
    <div class="row">
      ...
      <table id="myTable"  class="table table-condensed" >
        ...
        $run=mysqli_query($con,"SELECT DISTINCT `email` from `incident` WHERE `current_assignee`!='' and `current_assignee`='$my_email' ");

        while($run2=mysqli_fetch_assoc($run)){
            $reciv_email=$run2['email'];

            $count++;


            $result22=mysqli_query($con,"SELECT DISTINCT `email` from `incident` WHERE `current_assignee`!='' and `current_assignee`='$my_email'");
            while($runner22=mysqli_fetch_assoc($result22)){

             $name22= $runner22['email'];



            echo "<tr><td class='text-center'> <a href='inbox2.php?email=".$reciv_email."#send'><div class='btn btn-info btn-block'>By: ".$name22."</div></a><br>";
            ...
            $result=mysqli_query($con,"SELECT * FROM `chat` where (`user_email`='$name22' AND `employee_email`='$my_email')  ORDER BY `time` DESC limit 1");
